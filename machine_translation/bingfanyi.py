...
class BingFanyi:
    # @description: 该类的主功能函数为translate()，可向必应翻译api发送POST请求，得到翻译的结果。
     ...
     def translate (self, queryText):
        ...
        # @return:若请求微软api服务器失败，则返回原文内容。
        requestBody = [{
            'Text' : queryText,
        }]
        translateText = json.dumps(requestBody, ensure_ascii=False).encode('utf-8')
        try:
            conn = http.client.HTTPSConnection(self.host)
            conn.request ("POST", self.path + "&to=" + self.ToLanguage, translateText, self.headers)
            ...
            print("\n")
            print("请求微软api服务器失败，且看错误原因：{0}".format(e))
            return queryText
